...
export default function AdminDashboard() {
    ...
    return (
        <div className="animate-in fade-in space-y-6 px-4 py-8 container mx-auto">
            <h1 className="text-2xl font-semibold mb-6">{t('welcome', { name: admin?.name || '' })}</h1>
                        ...
                        <div className="flex items-center justify-between">
                            <div className="flex items-center gap-4">
                                ...
                                <div>
                                    ...
                                    <p className={`text-sm font-medium transition-colors duration-300 ${isOpen ? 'text-emerald-500' : 'text-red-500'
                                        }`}>
                            ...
                            <button
                                ...
                                className={`group relative flex items-center gap-3 rounded-full px-6 py-3 font-semibold text-sm transition-all duration-300 ${isToggling ? 'opacity-50 cursor-wait' :
                                        isOpen
                                            ? 'bg-red-500/10 text-red-500 border border-red-500/30 hover:bg-red-500/20 hover:shadow-lg hover:shadow-red-500/10'
                                            : 'bg-emerald-500/10 text-emerald-500 border border-emerald-500/30 hover:bg-emerald-500/20 hover:shadow-lg hover:shadow-emerald-500/10'
                                    }`}
                            >
            ...
            <div className="grid gap-4 sm:grid-cols-3">
                <Card className="bg-amber-500/10 border-amber-500/30">
                    <CardContent className="p-4 text-center">
                        ...
                        <p className="text-sm text-amber-600/80 dark:text-amber-300">{t('newOrders')}</p>
                    </CardContent>
                </Card>
                <Card className="bg-blue-500/10 border-blue-500/30">
                    <CardContent className="p-4 text-center">
                        <p className="text-3xl font-bold text-blue-500">{preparingOrders}</p>
                        <p className="text-sm text-blue-600/80 dark:text-blue-300">{t('preparing')}</p>
                    </CardContent>
                </Card>
                <Card className="bg-emerald-500/10 border-emerald-500/30">
                    <CardContent className="p-4 text-center">
                        <p className="text-3xl font-bold text-emerald-500">{readyOrders}</p>
                        <p className="text-sm text-emerald-600/80 dark:text-emerald-300">{t('ready')}</p>
                    </CardContent>
                </Card>
            </div>
        </div>
